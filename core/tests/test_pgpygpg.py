...
import os
import pytest
from autocrypt.experiments.pgpygpg import cached_property, PGPYGPG, KeyInfo
...
def test_pgpygpg_native(pgpygpg_maker, monkeypatch):
    pgpygpg1 = pgpygpg_maker()
    monkeypatch.setenv("GNUPGHOME", pgpygpg1.homedir)
    pgpygpg2 = pgpygpg_maker(native=True)
    assert not pgpygpg2.homedir
    keyhandle = pgpygpg2.gen_secret_key("x@y.org")
    keyinfos = pgpygpg1.list_public_keyinfos(keyhandle)
    for k in keyinfos:
...
class TestPGPYGPG:
    def test_failed_invocation_outerr(self, pgpygpg2):
        with pytest.raises(pgpygpg2.InvocationFailure):
            pgpygpg2._gpg_outerr(["qwe"])

    def test_gen_key_and_check_packets(self, pgpygpg):
        keyhandle = pgpygpg.gen_secret_key(emailadr="hello@xyz.org")
        keydata = pgpygpg.get_secret_keydata(keyhandle)
        packets = pgpygpg.list_packets(keydata)
