...
class BinGPG(object):
    ...
    def gen_secret_key(self,
        ...
        # the emailadr to be the e-mail address.  If name attribute is set to
        # empty string and email to the e-mail address, the emailadr will be '
        ...
        keyhandle = self._find_keyhandle()
    ...
    def _find_keyhandle(self, string, _pattern=re.compile("key (?:ID )?([0-9A-F]+)")):
        # FIXME: PGPy does not manage key rings
        logging.debug('')
